...
- **Invoice Management**: Create, edit, duplicate, and send invoices with configurable numbering, issue-date filtering, bulk status/accountant updates, customizable list columns, ZIP downloads of selected PDFs, and previewed CSV migration from Fakturoid
- **Recurring Invoices**: Monthly recurring invoice templates with optional auto-send
- **Expense Tracking**: Create and edit expenses, filter by issue date, bulk-update payment status, customize list columns, download selected source documents as a ZIP, import Alza PDFs, and batch-import any supplier invoices from structured text
...
- **Email Integration**: Send invoices via SMTP with customizable subject and message templates, optionally forward a separately templated copy to an accountant, track accountant delivery per invoice, and receive bank notifications via IMAP
