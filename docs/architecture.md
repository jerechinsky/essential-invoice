...
- **Universal expense import**: `services/universalInvoiceParser.ts` parses strict, human-readable invoice blocks generated from any supplier document. The route validates every block and exact filename pairing before writes, creates or reuses supplier contacts, then saves each expense with its PDF/JPEG/PNG source attachment. Aggregate totals support rounding and effective VAT rates for mixed-rate invoices.
- **Services**: `services/` - Business logic:
  ...
  - `emailSender.ts` - Per-user SMTP email sending with configurable client/accountant subject and message templates
  ...
  - `fakturoidCsvImport.ts` - RFC-style CSV parsing, row validation, status/amount mapping, idempotent invoice migration, and missing-contact creation for Fakturoid exports
...
- **Database**: PostgreSQL with `pg` driver. Schema managed in `db/init.ts` using idempotent CREATE TABLE IF NOT EXISTS and inline ALTER TABLE migrations (no separate migration files). `db/migrate.ts` is the migration runner script. Users table includes `vat_payer` (BOOLEAN, default false) for VAT payer status, `onboarding_completed` (BOOLEAN, default false) to track new-user onboarding, `language` (VARCHAR(5), default 'cs') for locale preference, and `pausalni_dan_enabled`/`pausalni_dan_tier`/`pausalni_dan_limit` for paušální daň settings. The settings table stores token-based invoice numbering in `invoice_number_format`, the first-invoice bootstrap value in `invoice_number_starting_sequence`, the independent monthly/yearly reset policy in `invoice_number_reset_period`, and the new-expense paid default in `default_expense_paid`. `password_reset_tokens` table stores hashed tokens for password reset flow. Invoices include EUR→CZK fields, `import_source` and `external_id` for idempotent external imports, and `accountant_email_sent_at` and `accountant_email_sent_to` for accountant-copy delivery tracking. Settings store the accountant address, template, and default-send preference. `exchange_rates` caches CNB rates by date and currency.
...
- **Pages**: `pages/` - Dashboard, Clients, ClientDetail, Invoices, InvoiceCreate, InvoiceDetail, RecurringInvoices, RecurringInvoiceCreate, RecurringInvoiceDetail, Expenses, ExpenseCreate, ExpenseDetail, Payments, Settings, Profile, Calculator, Login, Register, Onboarding, ForgotPassword, ResetPassword. Invoice and expense lists support issue-date presets/custom ranges, checkbox/Shift-range selection, persisted column visibility preferences, batch status updates, and ZIP downloads; invoices additionally support editing accountant-delivery state.
- **Utils**:
